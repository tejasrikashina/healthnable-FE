...
import { ArrangeLayoutComponent } from './arrange-layout/arrange-layout.component';
...
datasetValue=[
  ...
  constructor(private fb: FormBuilder,private _healthnableCoreService: HealthnableCoreService, private dialog:MatDialog,
    private router:Router,private _healtnableCoreService:HealthnableCoreService) {}
  ngOnInit() {
    ...
    this._healthnableCoreService.gridEditOption.subscribe((res) => {
      if (res.tableName === 'widget') {
        this.router.navigate(['/dashboards/dashboard/edit-widget']);
      }
    });
  }

arrange(){
    // let dialogRef = this.dialog.open(ArrangeLayoutComponent, {
    //       maxWidth: '1000px',
    //       disableClose: true,
    //     });
    //     dialogRef.afterClosed().subscribe((res: any) => {});
      const kpiCards = this.dataList.map((item, index) => ({
    header: item.kpi,
    isGraph: false,
    index: index
  }));

  const dialogRef = this.dialog.open(ArrangeLayoutComponent, {
    maxWidth: '1000px',
    disableClose: true,
    data: { cards: kpiCards }
  });

  dialogRef.afterClosed().subscribe((reorderedCards: any[]) => {
    if (reorderedCards && reorderedCards.length) {
      console.log('Reordered KPI Cards:', reorderedCards);
    }
  });
