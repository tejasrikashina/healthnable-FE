...
@Component({
  selector: 'app-rolebased-dashboard',
  standalone: true,
  imports: [MaterialModule,HeaderComponent,ReactiveFormsModule,TranslateModule,SelectComponent,TableComponent, PrimeNGModule],
  templateUrl: './rolebased-dashboard.component.html',
  styleUrl: './rolebased-dashboard.component.scss'
})
export class RolebasedDashboardComponent implements OnInit {
  constructor(private fb:FormBuilder,private _healthnableCoreService: HealthnableCoreService, private dashboardSer:DashboardService,
    private translate: TranslateService, private router:Router,private dialog:MatDialog,){
...
facilityValue: DropdownOption[] = [];
  ngOnInit() {
    ...
    this._healthnableCoreService.gridEditOption.subscribe((res) => {
      if (res.tableName === 'rolebased_dashboard') {
        this.router.navigate(['/dashboards/dashboard/edit-roleBased']);
      }
    });
    ...
    if (checked) {
      ...
      if (index !== -1) {
        ...
        if (!this.assignedDashboards.includes(dashboardName)) {
          this.assignedDashboards.push(dashboardName);
        }
      }
    } else {
